...
import { Roles, Resource, Actions, Scopes, PermissionsMatrix } from '../../types';
import { baseRoles } from '../../helpers';
import { AuthorizerTreatAsMap, AuthorizerTreatAs } from '../../authorizer-treat-as.directive';
...
const matrixAsClass = new Permissions();
matrixAsClass.allow({
  role: Roles.ADMIN,
  at: Resource.Division,
  to: [Actions.UPDATE, Actions.QUERY, Actions.APPROVE]
});
matrixAsClass.allow({
  role: Roles.ADMIN,
  at: Resource.User,
...
matrixAsClass.allow({
  role: Roles.USER,
  at: Resource.Division,
  to: [Actions.QUERY, Actions.APPROVE]
});
matrixAsClass.allow({
  role: Roles.PENDING,
  at: Resource.Division,
...
describe.each([['as object', matrixAsObject], ['as class', matrixAsObject]])(
  'Given: matrix defined as %s',
  (_description, matrixDef) => {
    const matrix = matrixDef as PermissionsMatrix | Permissions;
