...
class Authorizer {
    ...
    authenticate() {
        const { roles, scopes, user } = jwt.verify(this.accessToken, this.secret);
        this.user = user;
        this.roles = roles;
        this.scopes = scopes;
        return !!this.roles;
    ...
    can(action, authorizable, matrix, attribute, resource) {
        ...
        for (const permissions of matrix) {
            for (const [role, group] of Object.entries(permissions)) {
                ...
                if (resource) {
                    const authorizableAttribute = resource === authorizable.constructor.name ? 'id' : `${resource.toLowerCase()}_id`;
