...
class Authorizer {
    ...
    authenticate() {
        const { roles, scopes, user, client } = jwt.verify(this.accessToken, this.secret);
        this.user = user;
        this.roles = roles;
        this.scopes = scopes;
        this.client = client;
            ...
            for (const [role, group] of Object.entries(permissions)) {
                ...
                if (!access) {
                    for (const [resourceWithPermissions, allowedActions] of Object.entries(group)) {
                        const authorizableAttribute = !resource && resourceWithPermissions === authorizable.constructor.name
                            ? 'id'
...
__decorate([
    ...
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Object)
], Authorizer.prototype, "canRefresh", null);
__decorate([
    decorators_1.Requires('authenticate'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object, Array, String, String]),
