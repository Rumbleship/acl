...
class Authorizer {
     ...
     * ```
     */
    can(action, authorizable, matrix, attributeResourceMap = authorizer_treat_as_directive_1.getAuthorizerTreatAs(authorizable)) {
        let access = false;
        if (this.inScope(types_1.Scopes.SYSADMIN)) {
            return true;
        }
        if (matrix instanceof permissions_matrix_1.Permissions) {
        ...
        for (const permissions of Array.isArray(matrix) ? matrix : [matrix]) {
            for (const [role, group] of Object.entries(permissions)) {
                const permissionedIdentifiers = new Set(this.roles[role] || []);
                for (const [resource, attributes] of attributeResourceMap.entries()) {
                    const actions = new Set(group[resource] || []);
                    for (const attr of attributes) {
                        if (permissionedIdentifiers.has(authorizable[attr]) && actions.has(action)) {
                            access = true;
                        }
                    }
                }
...
__decorate([
    required_decorator_1.Requires('authenticate'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object, Object, authorizer_treat_as_directive_1.AuthorizerTreatAsMap]),
